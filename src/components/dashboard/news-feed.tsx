      ...
      <div className="news-list">
        {pageSignals.map((signal) => (
          <button className={`news-card ${signal.id === selectedId ? 'active' : ''}`} key={signal.id} onClick={() => onSelect(signal.id)}>
            ...
            <h3>{signal.title}</h3><p>{signal.summary}</p><div className="news-tags">{signal.tags.map((tag, index) => <span className={`tag ${index === 0 && signal.priority > 85 ? 'hot' : ''}`} key={tag}>{tag}</span>)}</div><SectorIndexContext signal={signal} />
          </button>
        ))}
