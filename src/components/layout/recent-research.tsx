...
export function RecentResearch({ pathname, reports }: { pathname: string; reports: PublicReport[] }) {
  ...
  const currentSlug = pathname.startsWith('/reports/') ? pathname.split('/')[2] : undefined;
  const slugs = typeof window === 'undefined' ? defaults : readRecentSlugs(currentSlug);
